...
export default function AIWorkerLayout({
	...
	const [avatarVolume, setAvatarVolume] = useState(1); // Default volume is 100%
...
`;
	...
	const handleMuteClick = () => {
		if (muted) {
			setMuted(false);
			// Minimum of 10% volume when unmuting:
			if (avatarVolume < 0.1) {
				setAvatarVolume(0.1);
			}
			setVolumeSliderValue(Math.round(avatarVolume * 100));
		} else {
			setMuted(true);
	...
	const [sliderOpacity, setSliderOpacity] = useState(0.1); // Initial opacity of 50%
	const [volumeSliderValue, setVolumeSliderValue] = useState(100); // This is used to set slider to 0 when muted, and set it to stored value when unmuting

	const handleAvatarVolumeChange = (event, newValue) => {
		console.log(`Volume: ${newValue}`);
		setVolumeSliderValue(newValue);
		setAvatarVolume(newValue > 0 ? newValue / 100 : 0);		// volume range on videoRef is 0.00 to 1.00
		setMuted(newValue === 0);
	};
			...
			<StyledAIInteractionPanel ref={chatPanelRef} style={{ width: `${chatPanelWidth}px` }}>
				...
				<StyledAIInfoWrapper>
					...
					<Stack pr={1} flex={1} justifyContent={'center'} alignItems={'flex-end'}
						...
						onMouseLeave={() => setSliderOpacity(0.1)}
						sx={{ cursor: 'pointer' }}
					>
						<Stack onClick={() => handleMuteClick()} justifyContent={'center'} alignItems={'center'} sx={{ width: '34px', height: '34px', borderRadius: '50%', backgroundColor: '#E3C7F9', cursor: 'pointer' }}>
							<Stack
							...
							>
								{muted ? (
									...
									// Show only VolumeUpIcon when avatarVolume is above 50
									avatarVolume > .5 ? (
										<VolumeUpIcon sx={{ color: 'white', width: '18px' }} />
									) : (
										// Show only VolumeDownIcon when avatarVolume is below or equal to 50
										<VolumeDownIcon sx={{ color: 'white', width: '18px' }} />
									))}
							</Stack>
						</Stack>
						<Slider
							...
							sx={{ opacity: sliderOpacity, maxWidth: 160 }}
							onChange={handleAvatarVolumeChange}
						/>
					</Stack>
				</StyledAIInfoWrapper>
