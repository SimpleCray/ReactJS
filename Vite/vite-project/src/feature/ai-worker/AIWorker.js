...
import { splitAndSortArray } from '../../utils/sortArray';
...
export default function AIWorker() {
	...
	const handleNotificationAction = useCallback(async (data) => {
		const target_url = await getSourceUrl({ source_type: 1, url: data.urlPath[0] });
		setAction({ name: 'notification', data });
		handleOutputCardWithClickRequestAction(undefined, 'VIEW_DOCUMENT', { name: data.data.document_name, document_category: data.data.document_type, sk: data.data.sk}, target_url)
	}, []);
