...
const CandidateDetails = ({ modalData }) => {
	...
	const { mutate: mutateEditNotes, isPending: isLoadingEditNotes } = useUpdateCandidateNotes({ endpoint: candidate?.endpoint });
    ...
    // Need to submit the notes separately
    ...
    const postNewCandidateData = async () => {
        ...
        Promise.all([
            AIPostAPIRequest(API_UPDATE_CLIENT_DETAILS, candidateData),
            onSubmitUpdateNotes(notes),
        ]).then((response) => {
    ...
    return (
        ...
        >
            ...
            <Stack
                ...
                onMouseOver={() => setHoveredSection('NOTES')}
                onMouseLeave={() => setHoveredSection(false)}
                gap={2}
                width={'100%'}
                flexDirection={'row'}
                alignItems={'flex-start'}
                sx={{ width: '100%', cursor: 'pointer', minHeight: '30px' }}
            >
                <Typography
                    variant={'body1'}
                    sx={{
                    ...
                    fontSize: '16px',
                    lineHeight: '22.4px',
                    color: activeSection === 'NOTES' ? '#9859E0' : '#21054C',
                    }}
                >
                    Notes
                </Typography>
                <TextEditorCommon
                    htmlContent={notes}
                    onSave={setNotes}
                    defaultIsEditMode={activeSection === 'NOTES'}
                    setDefaultIsEditMode={setActiveSection}
                    saveOnMouseLeave
                    isRequired={false}
