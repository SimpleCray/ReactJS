...
const JobOpeningOutputCard = ({
	...
	return (
		<OutputCard showTitle={false} title={!job_title && !client_name ? '-': `${job_title || '-'} ${client_name || '-'}`} showActions={false} headerAction={handleHeaderAction} headerActionIcon={<MenuIcon />} showHeaderMenu isATSCard closeCard={() => handleCardClose({ id: outputCardId })}>
				...
				<Stack flexDirection={'row'} alignItems={'center'} gap={2} sx={{ width: '100%' }}>
					...
					<Stack gap={0.5} flexWrap={'wrap'} flexDirection={'row'} alignItems={'center'} sx={{ whiteSpace: 'nowrap' }}>
						{!job_title && !client_name ? (
							<Typography sx={{ fontWeight: '400', fontSize: '1.125rem' }}>-</Typography>
						) : (
							<>
