...
import { useUpdateCandidateNotes } from 'src/hooks/PreScreeningSummary/usePreScreeningSummary';
...
const CandidateProfileOutputCard = ({
  ...
  const [editingNote, setEditingNote] = useState(false);
	...
	const { mutate: mutateEditNotes, isPending: isLoadingEditNotes } = useUpdateCandidateNotes({ endpoint: candidateNotesEndpoint.current });

	const setNotes = (notes) => {
		setCandidateDetails(prevState => ({...prevState, notes}))
	}
							...
							.then((response) => {
								if (response !== null) {
									if (section_name === 'CANDIDATE_DETAILS') {
										...
										setCandidateDetails({...candidate, notes: ''});
									...
									if (section_name === 'CANDIDATE_NOTES') {
										const notesEndpoint = `${process.env.API_CANDIDATE_NOTE}/${target_path}`;
										candidateNotesEndpoint.current = notesEndpoint;
										fetchNotes();
									}
	...
	const handleHeaderAction = (e) => {
		...
		window.sessionStorage.setItem('candidate_details', JSON.stringify({...candidateDetails, endpoint: candidateNotesEndpoint.current}));
	...
	return (
		<OutputCard sx={{ display: candidateDeleted && 'none' }} title={`${name || firstName || lastName || ''}`} titleIcon={<PersonOutlineOutlinedIcon />} showActions={false} showHeaderMenu={!candidateDeleted} headerActionIcon={<MenuIcon />} headerAction={handleHeaderAction} isATSCard closeCard={closeThiscard}>
			...
			<Stack direction='column' gap={1.5}>
				...
				<Details contact={name} email={email} phone={phone} showContactBox={showContactBox} showContact={false} active={candidateDetails?.candidate_status_id === 1} handleToggleActive={handleToggleActive}/>
