...
const PaymentCard = ({ cardName, cardSubtitle, paymentAmount, paymentIntervalTag, CTAText, CTALink, cardPointers, cardWidth }) => {
	...
	return (
		<Box maxWidth={{ sm: '500px' }} sx={{ background: 'linear-gradient(180deg, rgba(255, 255, 255, 0.15) 0%, rgba(255, 255, 255, 0.05) 50%, rgba(255, 255, 255, 0.15) 100%)', boxShadow: '0px 12px 24px 0px rgba(0, 0, 0, 0.10), 0px 12px 24px 0px rgba(0, 0, 0, 0.25)', borderRadius: '24px', minWidth: '0px', width: cardWidth ? cardWidth : '100%', padding: 4 }}>
			...
			<Stack direction={'row'} alignItems={'flex-end'} my={2}>
				<Box display={'flex'} flexDirection={'row'} mr={2} alignItems={'flex-start'}>
					<Typography component={'span'} sx={{ fontSize: '20px', alignSelf: 'center' }} color='white'>
						USD
					</Typography>
